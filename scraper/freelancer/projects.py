...
from core.config import freelancer_settings as settings
...
class FreelancerProjectsScraper(BaseProjectScraper):
    ...
    def _extract_bids(self, row: WebElement) -> int:
        """Extract number of bids from project row."""
        try:
    ...
    def _extract_price(self, row: WebElement) -> tuple[int, str]:
        ...
        try:
            ...
            *prices, currency = price_text.split()
            ...
            logger.warning(f"Failed to extract price, using default: {e}")
    ...
    def scrape_projects_list(self, page: int = 1) -> list[CreateProjectSchema]:
        ...
        """
        try:
            url = f"{settings.PROJECTS_PAGE}&page={page}"
